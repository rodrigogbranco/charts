...
nvec = {
    "sp": {1: 7016, 2: 12842, 3: 17500, 4: 22154, 5: 25882},
    "ny": {1: 4265, 2: 7789, 3: 10799, 4: 13622, 5: 16454},
}
...
evs_name = {"veh11651": "EV", "veh4856": "EV", "vehev1": "EV"}
...
def get_values(df_sce, metric, scenario):
    ...
    count = dict(
        [(i, df_sce[df_sce["instance"] == i][metric].count()) for i in instances]
    )
    means = dict(
        [(i, df_sce[df_sce["instance"] == i][metric].mean()) for i in instances]
    )
    std_err = dict(
        [(i, st.sem(df_sce[df_sce["instance"] == i][metric])) for i in instances]
    )
    y_errors = [
        means[i] - st.t.interval(0.95, count[i] - 1, loc=means[i], scale=std_err[i])[0]
        for i in instances
    ]
...
def make_line_graph(df, scenario):
    ...
    fig.add_trace(
        go.Scatter(
            x=x_values,
            y=means,
            name="Time-Loss/Actual Travel Time (%)",
            error_y=dict(
                type="data",  # value of error bar given in data coordinates
                array=y_errors,
                visible=True,
    ...
    # Add figure title
    fig.update_layout(title_text="No Preemption Results - {}".format(scenario))
    ...
    fig.update_yaxes(
        title_text="Time-Loss/Actual Travel Time (%)", secondary_y=True, range=[0, 100]
    )
...
def make_bar_graph(df, scenario, alg):
    ...
    fig.add_trace(
        go.Bar(
            name="TPN", x=x_values, y=means, error_y=dict(type="data", array=y_errors)
        )
    )
...
def make_boxplot_grouped(
    ...
    for alg in [alg for alg in algs_order if alg in algs]:
        ...
        fig.add_trace(go.Box(y=complete_values, x=xlabels, name=algs_names[lang][alg]))
